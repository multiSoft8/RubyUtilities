...
historyValueFile = "/var/tmp/historyPPPValue.txt"
lastIpValue = nil
ipChangeText = nil
historyText = nil
...
if !File.exist?(historyValueFile)
	fileHandle = File.new(historyValueFile, "w")
	fileHandle.close
end
...
#	puts "new=#{ipAddress}|"
	if ipAddress.eql?(lastIpValue)
		...
		fileHandle.close
		fileHandle = File.new(historyValueFile, "a")
		fileHandle.puts(ipAddress)	
		fileHandle.close
	end
	fileHandle = File.new(historyValueFile, "r")
	fileTempText = fileHandle.read
	fileHandle.close
	tempArray = fileTempText.split("\n")
	historyText = "History, ip count #{tempArray.count}"
end
puts "Interface and ip found int=#{interface} ip=#{ipAddress}. #{ipChangeText} #{historyText}"
exit OKV
